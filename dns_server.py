...
def send_dns_query(server, domain):
    ...
    try:
        ...
        print(f"Error in send_dns_query: {type(e).__name__} - {e}")
...
def extract_next_nameservers(response):
    ...
    for ns_name in ns_names:
        try:
            # Use dnspython's resolver to get the IP addresses
            answers = dns.resolver.resolve(ns_name)
            for rdata in answers:
                ns_ips.append(rdata.address)
                print(f"Resolved {ns_name} to {rdata.address}")
        except dns.resolver.NXDOMAIN:
            print(f"Could not resolve nameserver {ns_name}: No such domain")
        except Exception as e:
            print(f"Error resolving nameserver {ns_name}: {type(e).__name__} - {e}")
...
def iterative_dns_lookup(domain):
    ...
    while next_ns_list:
        ...
        if response: #checks if response is not NONE
            ...
            new_ns_list = extract_next_nameservers(response)

            # Update the stage (logic based on the nameservers you are using)
            if stage == "ROOT":
                stage = "TLD"
            elif stage == "TLD":
                stage = "AUTH"

            next_ns_list = new_ns_list  # Update nameserver list
...
def recursive_dns_lookup(domain):
    ...
    try:
        # to perform the recursive resolution using the system's DNS resolver
        answers = dns.resolver.resolve(domain, "A")
        for rdata in answers:
            print(f"[SUCCESS] {domain} -> {rdata.address}")
    except dns.resolver.NXDOMAIN:
        print(f"[ERROR] The domain {domain} does not exist.")#Added specific error handling
    except dns.resolver.NoNameservers:
        print(f"[ERROR] No nameservers are available to resolve the domain {domain}.")
    except Exception as e:
        print(f"[ERROR] Recursive lookup failed: {type(e).__name__} - {e}")

if __name__ == "__main__":
    import sys

    # Test the send_dns_query function
    #test_response = send_dns_query("198.41.0.4", "example.com")  # Query a root server for example.com
    #if test_response:
    #    print("send_dns_query test successful!")
    #else:
    #    print("send_dns_query test failed.")
    ...
    # to execute the selected DNS resolution mode
    if mode == "iterative":
        iterative_dns_lookup(domain)
    else:
        recursive_dns_lookup(domain)
    
    print(f"Time taken: {time.time() - start_time:.3f} seconds")  # print dexecution time